...
export default function TaxEstimate({ estimate, taxYear, pendingIncome = 0 }: Props) {
  const completedIncome = estimate.grossIncome - pendingIncome

  const rows: TaxRow[] = [
    { label: 'Completed Income', amount: completedIncome },
    ...(pendingIncome > 0 ? [{ label: 'Pending / Upcoming', amount: pendingIncome, note: 'projected', muted: true }] : []),
    { label: 'Projected Gross Income', amount: estimate.grossIncome, highlight: true },
    { label: 'Personal Allowance', amount: -estimate.personalAllowance, note: '2024/25' },
    { label: 'Taxable Income', amount: estimate.taxableIncome },
    ...
    { label: 'Projected Take-Home', amount: estimate.takeHome, highlight: true },
  ]
    ...
    <div className="card overflow-hidden">
      <div className="bg-[#2B2B2B] px-4 py-3">
        ...
        <p className="text-white/50 text-xs mt-0.5">
          {pendingIncome > 0 ? 'Projected figure includes pending/upcoming earnings. ' : ''}Estimate only — consult a qualified accountant for advice.
        </p>
      </div>
      ...
      <div className="divide-y divide-gray-100">
        {rows.map(row => (
          <div key={row.label} className={`flex items-center justify-between px-4 py-2.5 ${row.highlight ? 'bg-gray-50' : ''}`}>
            <div>
              <span className={`text-sm ${row.highlight ? 'font-semibold text-[#2B2B2B]' : row.muted ? 'text-gray-400' : 'text-gray-600'}`}>
                {row.label}
              </span>
